...
public:
	...
	ImageAndExposure* getImage(std::string fileName, bool forceLoadDirectly=false)
	{
		return getImage_internal(fileName, 0);
	}
...
private:
	...
	MinimalImageB* getImageRaw_internal(std::string fileName, int unused)
	{
		if(!isZipped)
		{
			// CHANGE FOR ZIP FILE
			return IOWrap::readImageBW_8U(fileName);
		}
		// else for file zip da sistemare...
		return NULL;
	}
	...
	ImageAndExposure* getImage_internal(std::string fileName, int unused)
	{
		MinimalImageB* minimg = getImageRaw_internal(fileName, 0);
		/*
		ImageAndExposure* ret2 = undistort->undistort<unsigned char>(
				minimg,
				(exposures.size() == 0 ? 1.0f : exposures[id]),
