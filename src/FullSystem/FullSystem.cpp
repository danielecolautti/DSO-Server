...
float depthMapScaleFactor = 1;
float max = -1;


FullSystem::FullSystem(std::string prefix,float scaleFactor) {
	...
	depthPrefix = prefix;
	depthMapScaleFactor = scaleFactor;
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
	...
	if(!initialized)
	{
		...
		{
			coarseInitializer->setFirst(&Hcalib, fh, getDepthMap(fh,frameNumber));
...
void FullSystem::initializeFromInitializerCNN(FrameHessian* newFrame)
{
	...
	cv::Mat depth = getDepthMap(firstFrame,frameNumber);
	frameNumber = frameNumber + 1;
	...
	void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	cv::Mat depth = getDepthMap(newFrame,frameNumber);
	frameNumber = frameNumber + 1;
    for (IOWrap::Output3DWrapper *ow : outputWrapper)
...
cv::Mat FullSystem::getDepthMap(FrameHessian* fh, int n){
	...
	std::string depthMapName = depthPrefix;
	int num = n;
	for (int p=0; p<5; p++) {
		num = num / 10;
		if (num == 0)
			depthMapName = depthMapName + "0";
	}
	depthMapName = depthMapName + std::to_string(n) + "_depth.png";
	//std::cout << depthMapName << std::endl;
	
	cv::Mat depthOriginal = cv::imread(depthMapName,cv::IMREAD_UNCHANGED);
	double minOriginal,maxOriginal;
	cv::minMaxLoc(depthOriginal,&minOriginal,&maxOriginal);
	
	cv::Mat depthFloat;
	
	depthOriginal.convertTo(depthFloat,CV_32FC1);
	
	double minFloat,maxFloat;
	cv::minMaxLoc(depthFloat,&minFloat,&maxFloat);

	// depthFloat = depthFloat / depthMapScaleFactor;
	
//	/*
	if (depthMapScaleFactor == 0) {
	    std::cout << "Inverto!" << std::endl ;
	    depthFloat = 255.0 - depthFloat;
	    
	    //depthFloat = depthFloat * depthMapScaleFactor / 14.5;
	    //depthFloat = 1.0 / (depthFloat + 0.0001);
		//depthFloat = depthFloat * 14.5;
		
		//double minLoc,maxLoc;
		//cv::minMaxLoc(depthFloat,&minLoc,&maxLoc);
		//depthFloat = 1 * (depthFloat - minLoc) / (maxLoc - minLoc);
		//depthFloat = 10 * depthFloat;
    } else if (depthMapScaleFactor < 0) {
		 depthFloat = 1.0 / (depthFloat + 0.0001);
		 float factor = depthMapScaleFactor * (-1.0);
		 depthFloat = depthFloat / factor;
		 if (n == 0) {
			double minFloatR,maxFloatR;
			cv::minMaxLoc(depthFloat,&minFloatR,&maxFloatR);
			std::cout << "\tFloatR: " << minFloatR << " , " << maxFloatR << " (Factor=" << factor << ")" << std::endl;
		 }
	} else
        depthFloat = depthFloat / depthMapScaleFactor;
//	*/
	...
	if (minOriginal == 0) {
		for(int y = 0; y < depthFloat.rows; ++y) {
			for(int x = 0; x < depthFloat.cols; ++x) {
				if (depthFloat.at<float>(y,x) < minNotZero && depthFloat.at<float>(y,x) != 0)
					minNotZero = depthFloat.at<float>(y,x);
			}
		}

		float lastNotZero=-1;
		for(int y = depthFloat.rows-1; y >= 0; --y) {
			for(int x = 0; x < depthFloat.cols; ++x) {
				if (depthFloat.at<float>(y,x) == 0) {
					depthFloat.at<float>(y,x) = lastNotZero == -1 ? minNotZero : lastNotZero;
					/*
					// ANGOLI
					if (x==0 && y==0)
						depth.at<float>(0,0) = (depth.at<float>(0,1) + depth.at<float>(1,0)) / 2.0;
					else if (x == depth.cols -1 && y == 0)
						depth.at<float>(0,depth.cols-1) = (depth.at<float>(0,depth.cols-2) + depth.at<float>(1,depth.cols-1)) / 2.0;
					else if (x == 0 && y == depth.rows - 1)
						depth.at<float>(depth.rows-1,0) = (depth.at<float>(depth.rows-2,0) + depth.at<float>(depth.rows-1,1)) / 2.0;
					else if (x == depth.cols -1 && y == depth.rows - 1)
						depth.at<float>(depth.rows-1,depth.cols-1) = (depth.at<float>(depth.rows-1,depth.cols-2) + depth.at<float>(depth.rows-2,depth.cols-1)) / 2.0;
					
					// CONTORNI (NO ANGOLI)
					else if (x == 0)
						depth.at<float>(y,0) = (depth.at<float>(y-1,0) + depth.at<float>(y,1) + depth.at<float>(y+1,0)) / 3.0;
					else if (y == 0)
						depth.at<float>(0,x) = (depth.at<float>(0,x-1) + depth.at<float>(1,x) + depth.at<float>(0,x+1)) / 3.0;
					else if (x == depth.cols-1)
						depth.at<float>(y,depth.cols-1) = (depth.at<float>(y-1,depth.cols-1) + depth.at<float>(y,depth.cols-2) + depth.at<float>(y+1,depth.cols-1)) / 3.0;
					else if (y == depth.rows-1)
						depth.at<float>(depth.rows-1,x) = (depth.at<float>(depth.rows-1,x-1) + depth.at<float>(depth.rows-2,x) + depth.at<float>(depth.rows-1,x+1)) / 3.0;
					
					// TUTTO IL RESTO
					else
						depth.at<float>(y,x) = (depth.at<float>(y,x-1) + depth.at<float>(y-1,x) + depth.at<float>(y,x+1) + depth.at<float>(y+1,x)) / 4.0;
					
					if (depth.at<float>(y,x) == 0) {
						depth.at<float>(y,x) = lastNotZero == -1 ? minNotZero : lastNotZero;
						std::cout << "Zero --> x=" << x << "\ty=" << y << "\tvalue=" << depth.at<float>(y,x) << std::endl;
					} 
					*/
				} else
					lastNotZero = depthFloat.at<float>(y,x);
			}
		}
	}
	
	double minFinal,maxFinal;
	cv::minMaxLoc(depthFloat,&minFinal,&maxFinal);
	
	// max = maxFinal > max ? maxFinal : max;
	
	/*
	std::cout << "Originale:  " << minOriginal << " , " << maxOriginal ;
	std::cout << "\tFloat: " << minFloat << " , " << maxFloat;
	std::cout	<< "\tFinal: " << minFinal << " , " << maxFinal << (minOriginal == 0 ? "\tC'era uno zero..." : "\tNo zeri.") << "\tMax attuale:" << max << std::endl;
	*/
	
	// USE TO CONVERT FROM DISPARITY TO DEPTH
	// depth = 0.3128f / (depth + 0.00001f);

	/*
	if (n == 0) {
		cv::Mat depthToWrite;
		depthFloat.convertTo(depthToWrite,CV_8UC3,255.0);
		cv::imwrite("/media/sf_Ubuntu/provaDepth.png",depthToWrite);
	}
	*/

	if ((n+1) % 50 == 0)
		std::cout << n+1 << " depth read!" << std::endl;
	return depthFloat;
}
